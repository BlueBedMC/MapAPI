 ...
 */
public class MapColorRenderer implements WindowRunnable {

    private final int fboID;
    private final GLFWCapableBuffer framebuffer;
    private final WindowRunnable renderCode;
    ...
    public MapColorRenderer(GLFWCapableBuffer framebuffer, WindowRunnable renderCode) {
        this(framebuffer, renderCode, MapColorRenderer.defaultFramebuffer(framebuffer.width(), framebuffer.height()));
    }

    public MapColorRenderer(GLFWCapableBuffer framebuffer, WindowRunnable renderCode, FboInitialization fboInitialization) {
    ...
    public void run(long window) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, fboID);
        renderCode.run(framebuffer.getGLFWWindow());
    ...
    private void loadPalette(String filename) {
        ...
        BufferedImage image;
        try {
            image = ImageIO.read(Objects.requireNonNull(MapColorRenderer.class.getResourceAsStream("/textures/" + filename + ".png")));
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static int createShader(String filename, int type) {
        ...
        try(BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(MapColorRenderer.class.getResourceAsStream(filename))))) {
